...
import { respondCampanhaInflu } from "@/lib/cliente-link.functions";
 ...
 * uma rota real dentro do mesmo shell do portal (sidebar continua visível,
 * sem topbar/abas novas), pra revisar perfil por perfil sem precisar abrir
 * cada um manualmente na galeria. Cada decisão salva imediatamente (mesma
 * `respondCampanhaInflu` de sempre) — "Voltar"/"Próximo" são só navegação
 * dentro de uma leva CONGELADA no início da sessão de revisão (ids fixados
 * em `frozenIds`), pra não reordenar a lista debaixo do cliente conforme
 * ele decide e o `reload()` tira cada perfil decidido de "aguardando".
 */
export const Route = createFileRoute("/portal/$token/campanhas/$campanhaId/revisar")({
  component: RevisarPerfisPage,
});

function RevisarPerfisPage() {
  const { token, campanhaId } = Route.useParams();
  const navigate = useNavigate({ from: Route.fullPath });
  const { data, reload, lang } = usePortalData();
  ...
  const goBack = () =>
    void navigate({ to: "/portal/$token/campanhas/$campanhaId", params: { token, campanhaId } });
  ...
  const decide = async (
    ...
    try {
      await respondInfluFn({
        data: { token, campanhaId, influencerId: inf.id, status, motivoLabel, comentario },
      });
      setRejectOpen(false);
      reload();
  ...
  if (!inf) {
    // O perfil desta posição sumiu dos dados (ex: removido pelo time
    // enquanto o cliente revisava) — pula pro próximo em vez de travar.
    advance();
    return null;
  }
  ...
  return (
    <PageContainer>
      ...
      {/* Rodapé fixo (sticky dentro do próprio scroll do shell, não `fixed`
          no viewport — a sidebar do portal é um item flex, não teria como
          um `fixed` full-bleed saber respeitar a largura dela) — vermelho
          reservado só pro botão de não aprovar. */}

 ...
 * mesma técnica do `.stretched-link` do Bootstrap), não um pai envolvendo
 * o conteúdo. O conteúdo visível (logo, nome, menu de três pontos) fica
 * numa div irmã por cima (`relative`, sem z-index — participa da mesma
 * camada de pintura que elementos posicionados com z:auto, na ordem do
 * DOM); só o botão de menu tem `z-10` pra ganhar de propósito do overlay
 * nessa pequena área e continuar clicável/focável de forma independente.
 * Resultado: clique/Enter/Espaço em qualquer área neutra abre o drawer;
 * três pontos/Editar/Excluir/Ver detalhes (todos fora do `<button>` de
 * overlay) nunca disparam essa navegação.
 */
export function ClienteCard({
  ...
  return (
    <div className="group relative cursor-pointer rounded-[20px] bg-card p-4 text-left transition-colors hover:bg-accent/40 dark:shadow-none">
      ...
      <div className="relative mt-4 flex flex-wrap items-center gap-x-4 gap-y-1.5 border-t border-border/60 pt-3 text-xs text-text-secondary">

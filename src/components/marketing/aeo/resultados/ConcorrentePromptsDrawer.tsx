...
import type { AeoPrompt } from "@/lib/aeo-store";

export function ConcorrentePromptsDrawer({
  concorrente,
  prompts,
  onClose,
}: {
  ...
  return (
    <Dialog open={!!concorrente} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>{concorrente}</DialogTitle>
        </DialogHeader>
        <ul className="max-h-80 space-y-1.5 overflow-y-auto text-xs">
          {prompts.length === 0 && (
            <li className="text-muted-foreground">Nenhum prompt encontrado.</li>
          )}
          {prompts.map((p) => (
